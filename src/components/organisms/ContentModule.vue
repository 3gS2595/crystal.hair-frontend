        ...
        <div class="grid">
          ...
          >
            <div v-if="item.id === 'page-load'" class="cont-load">
              <img
                class="nav-loader-icon"
                src="https://crystal-hair.nyc3.cdn.digitaloceanspaces.com/page-loader.gif"
              />
            </div>
            <div v-else>
...
<script setup lang="ts">
  ...
  const observer = new IntersectionObserver(entries => {
    for (const entry of entries) {
      if (
        ...
        kernals.value[kernals.value.length - 1].signed_url_s !==
          'https://crystal-hair.nyc3.cdn.digitaloceanspaces.com/page-loader.gif'
      ) {
        observer.disconnect()
  ...
  const watchIntersect = () => {
    observer.disconnect()
    for (let i = 1; i < 5; i++) {
      const el = document.getElementsByClassName('cgb-0')[
        (pageNumber.value - 1) * store.pageSize - 5 * i
      ] as HTMLElement
      if (el) {
        observer.observe(el)
      }
